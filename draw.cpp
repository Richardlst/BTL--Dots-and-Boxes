...
//Hàm loadMedia được sử dụng để tải và hiển thị các tệp hình ảnh trên cửa sổ đồ họa của SDL.
//Hàm này có ba tham số đầu vào là ob, x và y.
//ob là số thứ tự của tệp hình ảnh cần tải lên.
//Các tệp hình ảnh này đã được tải lên trong hàm initWindow() và được lưu trữ trong mảng texture.
//x và y là tọa độ cần hiển thị hình ảnh trên cửa sổ đồ họa.
//Trong hàm, một đối tượng SDL_Rect được tạo ra với tọa độ và kích thước hình ảnh cần hiển thị.
//Sau đó, hàm SDL_RenderCopy được sử dụng để sao chép nội dung của texture[ob]
//(tức là tệp hình ảnh được lưu trữ trong mảng texture) vào vùng nhớ được chỉ định bởi dest trên cửa sổ đồ họa.
